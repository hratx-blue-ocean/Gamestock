...
INNER JOIN users
ON items_in_collection.user_id = users.id AND users.username='${username}'
ORDER BY items.current_price DESC`
  );
};
...
const getAllConsolesByUser = (username) => {
  selectQueryUserConsoles = `SELECT DISTINCT console FROM items
  INNER JOIN items_in_collection
  ON items.id = items_in_collection.item_id
  INNER JOIN users
ON items_in_collection.user_id = users.id AND users.username='${username}'`;

  return pool.query(selectQueryUserConsoles);
};
